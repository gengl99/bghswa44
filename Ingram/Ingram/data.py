...
class Data:
            ...
            with open(state_file, 'r') as f:
                if line := f.readline().strip():
                    try:
                        ...
                        logger.warning(f"Invalid state file {state_file}: {e}")
    ...
    def add_vulnerable(self, item):
        with self.vulnerable_lock:
            self.vulnerable.writelines(','.join(str(x) for x in item) + '\n')
            self.vulnerable.flush()

    def add_not_vulnerable(self, item):
        with self.not_vulneralbe_lock:
            self.not_vulneralbe.writelines(','.join(str(x) for x in item) + '\n')
